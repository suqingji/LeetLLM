// engine.rs
// ==============================================================================
// 统一 BPE 引擎：编码 + 解码合体，共享一次文件 I/O
// ==============================================================================
// 重构动机：
//   旧架构中 Tokenizer（encode.rs）和 Decoder（decode.rs）各自独立初始化，
//   Python 端要手动读 vocab.json / merges.txt、解析嵌套格式、剥离特殊 token、
//   逐行 split merges 并查字典拼元组——这些全是 Python 最慢的字符串操作。
//   现在全部内化到 Rust：
//     1. serde_json 直接读 vocab.json（自动处理嵌套格式）
//     2. Rust 内部逐行解析 merges.txt 并查 vocab 拿 ID
//     3. 一次遍历 vocab 同时构建编码侧和解码侧的数据结构
//     4. 特殊 token 的判断和分流完全在 Rust 内部完成
//
// Python 端用法（从 50+ 行变成 2 行）：
//   engine = custom_bpe.BpeEngine("path/to/vocab.json", "path/to/merges.txt")
//   ids = engine.encode("床前明月光")
//   text = engine.decode(ids)
//   chunk = engine.decode_stream(next_id)
//   engine.reset_stream()
//
// 性能优化全部保留：
//   - FxHashMap：极速非加密哈希，小整数键 O(1) 查找
//   - Aho-Corasick 自动机：O(N) 线性时间扫描所有特殊词元
//   - 双路径 BPE 合并：短词走 ranks 数组扫描，长词走数组双向链表 + BinaryHeap
//   - 零分配 Buffer：循环外预分配，热路径只做 clear()
//   - GIL 释放：编码/解码期间不阻塞 Python 其他线程
//   - 流式解码：自动处理"半个汉字"的 UTF-8 字节缓冲
// ==============================================================================
