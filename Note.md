...
如果说**线性插值**是把弹簧暴力压短，**NTK** 是把弹簧上疏下密地压短，那么 **YaRN** 就是：**最紧密的那段弹簧完全不动，最松散的那段弹簧均匀压短，中间过渡，最后再给弹簧喷上一层定型胶（注意力缩放）防止它散架。** 这也是目前开源界扩展上下文窗口最主流、最优雅的技术方案之一。
...
知识蒸馏（Knowledge Distillation, KD）是机器学习中一种经典的“模型压缩”和“能力迁移”技术。

核心思想是：**让一个庞大、复杂的“教师模型（Teacher Model）”来指导一个轻量、简单的“学生模型（Student Model）”进行训练。** 其目的是让学生模型在保持较小参数量和较快推理速度的同时，尽可能获得接近教师模型的性能。



最经典的知识蒸馏方法（由 Geoffrey Hinton 提出）主要通过“软标签（Soft Targets）”来实现。

在普通的分类任务中，数据只有“硬标签（Hard Targets）”（比如：100%是猫，0%是狗）。但教师模型在输出时，会给出一个概率分布（比如：85%是猫，10%是狗，5%是汽车）。这个分布包含了教师模型学到的“暗知识（Dark Knowledge）”——它告诉学生模型，猫不仅是猫，它在视觉特征上更像狗，而不像汽车。



白盒蒸馏是指**学生完全可以访问教师模型的内部结构和所有参数**。不仅能看到教师的最终输出（Logits），还能看到它的中间隐藏层（Hidden States）、注意力矩阵（Attention Maps）甚至梯度信息。

黑盒蒸馏是指**学生完全无法访问教师模型的内部结构和权重**，只能把它当成一个“黑盒 API”。学生只能给教师发送输入（Prompt），然后接收教师的最终输出（Response）。在当前的大语言模型（LLM）时代，这通常意味着利用闭源巨头（如 GPT-4、Claude 3.5）来训练开源小模型。
...
在过去，开发者倾向于将模型的对话能力（Chat）和深度思考能力（Reasoning）解耦，甚至维护两套权重。但随着技术的发展，业界发现这种物理隔离不仅增加了维护成本，而且在算法层面上是多余的。推理（Reasoning）能力正在从“特化分支”回归到“通用对齐（Alignment）”的主干中。

推理训练的核心目的是激发大模型的“慢思考”（System 2）能力，使其在给出最终结论前，先生成连续的逻辑推导步骤（Chain-of-Thought, CoT）。在实现层面，这并不是一种全新的基础架构变动，而是**数据工程、损失函数设计与强化学习的结合**。它的主流训练链路通常包含以下几个阶段：

监督微调（SFT）阶段：冷启动与格式学习

在这一步，并不是去改变 Transformer 的底层运算，而是通过强迫模型学习特定的文本范式。

- **数据构造：** 将高质量的逻辑推导过程（如数学解题、代码编写、物理公式推导）封装在特定的标签中。 范例数据格式：`用户输入 -> <think> 步骤1... 步骤2... 修正错误... </think> <answer> 最终结果 </answer>`
- **目标：** 让模型在参数空间中建立一种条件反射——当遇到复杂问题或被特定的 `chat_template` 触发时，优先输出 `<think>` 标签，并在闭合标签前持续生成中间推演，而非急于输出最终概率分布的“快思考”答案。

强化学习（RL / RLAIF）阶段：效果对齐与能力泛化

单纯的 SFT 容易让模型“模仿”思考的格式，而不是真正去“解决”问题。推理训练的重头戏在于强化学习（如 PPO 或 GRPO 算法）。

- **规则奖励（Rule-based Reward）：** 针对数学或代码等有明确对错的领域，如果最终 `<answer>` 是正确的（例如单元测试通过、方程左右两边平衡），给予高奖励。
- **格式奖励（Format Reward）：** 强制模型必须遵循 `<think>...</think>` 的闭合结构，否则给予惩罚。
- **过程监督（PRM）：** 奖励模型在 `<think>` 过程中展现出的自我纠错（Self-correction）和回溯能力。

通过这两个阶段，模型学会了利用生成 `<think>` 内容的过程，作为其注意力机制（Self-Attention）展开复杂计算的“缓存区”。



将独立的 `reason_*.pth` 废弃，统一并入标准 SFT / RLAIF 流程，是工程与算法成熟后的必然结果。主要原因有以下几点：

推理本质上是一种“格式化输出”与“状态切换”

从计算图和算子（Operators）的角度来看，生成“你好”和生成一段复杂的 CUDA Kernel 优化逻辑，底层都是 Next-token 预测。推理并不需要特殊的网络结构。 通过引入 `chat_template` 和 `open_thinking` 开关，我们可以在推理（Inference）和训练（Training）阶段，动态地向 System Prompt 中注入系统指令。模型只需要一套权重，就可以根据开关状态，在“直接回复”和“显式思考”两种状态间丝滑切换。

避免灾难性遗忘，降低对齐税（Alignment Tax）

当分别维护 `chat.pth` 和 `reason.pth` 时，经常会遇到一个痛点：

- 优化了逻辑推理权重的模型，往往会丧失日常对话的自然度和角色扮演能力。
- 优化了对话权重的模型，逻辑严密性又会下降。 现有的 RLAIF 框架已经足够强大，能够在同一个损失函数中联合优化多个目标。通过混合数据采样（将日常对话数据与深度推理数据按比例混合训练），可以在单一权重中实现能力共存，大幅降低维护两套管线的算力和显存成本。

训练基础设施的统一

当前主流的训练框架（如 Llama-Factory、Megatron 等）已经将 `<think>` 标签的解析、Reward Model 的打分逻辑做到了底层抽象中。 开发者不再需要写一个专门的 `train_reason.py` 来处理特殊的数据 Loader 和 Loss 计算。你只需要在标准的训练配置（如 YAML 文件）中，指定 `dataset_mix` 并开启 `enable_reasoning_tags: true`，底层系统就会自动在计算梯度和更新权重时处理好掩码（Masking）和奖励回传。



早期的解耦推理训练（Decoupled Reasoning Training）核心思路是“物理隔离”——将通用对话和逻辑推理作为两个完全独立的任务，训练出两套不同的模型权重（例如 `chat.pth` 和 `reason.pth`）。具体的做法通常可以拆解为以下四个维度的解耦：

数据源与预处理的解耦：在独立的 `train_reason.py` 阶段，数据加载器（Data Loader）是高度特化的，完全排除了日常对话数据。

损失函数（Loss）与掩码（Masking）的解耦：通用的对话训练（SFT）通常是对模型生成的所有回复 token 计算交叉熵损失（Cross-Entropy Loss）。但在解耦的推理训练中，`train_reason.py` 会做特殊处理：有时为了只让模型关注最终结果的准确性，会把前面推导过程的 Loss Mask 掉（即不参与梯度回传），只针对最终的答案计算 Loss。**或反之（过程监督）：** 强制要求模型严格按照某一种特定的解题范式（如 CoT 模板）输出，专门针对推导步骤计算极高的权重 Loss，而对最终答案的格式不做过度干预。

部署与推理机制的解耦（Router 模式）：既然训练出了两套权重，在实际部署时，开发者不得不采用“路由分发”（Routing / Dispatching）的架构

这种解耦做法在早期确实能快速刷高数学和代码榜单的分数，但工程和实用代价极其高昂：

1. **割裂的上下文：** 如果用户在同一个会话中，先寒暄几句，然后立刻问了一个复杂的物理推导问题。由于会话被强行切分给两个模型，`reason.pth` 往往没有“多轮对话”的能力，它接不住之前的上下文，导致体验极差。
2. **维护成本翻倍：** 开发者需要维护两套数据管线、两套训练脚本（`train_chat.py` 和 `train_reason.py`），部署时还要多占一倍的显存来同时挂载两个模型，或者容忍模型切换带来的巨大延迟。
3. **算法边界融合：** 后来研究发现，高质量的对话本身也需要逻辑支撑，而枯燥的推理如果能用自然语言很好地解释出来，其泛化能力更强。物理隔离实际上阻碍了模型将“逻辑能力”向“通用能力”迁移。

个人思考：

其实早期的就像两个模型，通过路由器分发传给两个模型参数，导致KV cache等技术没法在两个模型中应用，也就丧失了长对话多次对话能力
...
LoRA 是一种常见的参数高效微调（Parameter-Efficient Fine-Tuning, PEFT）方法。相比全参数微调，它只更新少量新增参数，而保留原始模型主体权重不变，因此训练成本更低，也更适合做垂直场景适配。 它的核心思想是在原有权重矩阵旁引入低秩增量分支，仅训练这部分低秩参数，从而用较小代价完成能力迁移。
...
这是基于人类反馈强化学习（RLHF）的经典算法，OpenAI 早期训练 ChatGPT 时广泛使用。PPO 需要训练一个奖励模型（Reward Model）来模仿人类的打分偏好。在强化学习阶段，模型（Actor）生成回答，奖励模型给出分数，PPO 根据这个分数来更新模型策略。为了防止模型“钻空子”过度追求高分而变得不说人话，PPO 会引入 KL 散度惩罚，限制每次更新的幅度（“近端”的含义）。

**优势：** 理论上限高，能够处理复杂的连续反馈，非常适合开放域的对话对齐。

**劣势：** 工程实现极其复杂，且对显存要求极高。训练时通常需要同时加载四个模型（Actor 模型、Reference 基础模型、Critic 价值模型、Reward 奖励模型）。
...
#### DPO (Direct Preference Optimization) 直接偏好优化
 ...
 PPO 太复杂了，DPO 的出现是为了“干掉”奖励模型。DPO 通过数学推导证明，强化学习中的奖励函数可以直接表示为最优策略与参考策略之间的对数概率差。因此，DPO 不需要单独训练奖励模型，也不需要强化学习的循环。它直接使用人类标注的“好回答”和“坏回答”对（Chosen vs. Rejected），通过一种二元交叉熵损失函数，直接在语言模型本身上进行优化。

**优势：** 极其稳定、轻量且易于实现。只需要加载 Actor 和 Reference 两个模型，大大降低了对齐阶段的算力和显存门槛。

**应用场景：** 目前开源社区（如 Llama-3、Qwen）在对齐人类偏好时，最广泛采用的方法。



#### GRPO (Group Relative Policy Optimization)群组相对策略优化

这是 DeepSeek 在训练 DeepSeekMath 和 DeepSeek-R1（推理模型）时提出并带火的 PPO 变体。传统的 PPO 需要一个同等体量的 Critic 模型来预测基线（Baseline），显存占用巨大。GRPO 摒弃了 Critic 模型。它的做法是：对于同一个提示词（Prompt），让模型生成一组（Group）多个不同的回答。然后根据规则（比如数学题结果是否正确）或奖励模型给这组回答打分，通过组内得分的**相对高低**（标准化处理）来决定哪些回答值得鼓励，哪些需要惩罚。

**优势：** 省去了 Critic 模型，大幅降低了训练强化学习时的显存开销，使得将更多算力投入到生成大量样本（Actor）上成为可能。

**应用场景：** 特别适合有着明确“对错”标准的任务（如数学推理、代码生成），是开启大模型“强化学习顿悟时刻（Aha Moment）”的关键算法。
...
Tool Calling 是一种**让大模型决定何时、如何使用外部工具（API、计算器、数据库等），并格式化输出调用指令的能力**。

大模型本质上是一个“文本接龙”机器，它的知识被封印在训练截止的那一天，且不擅长精确的数学计算或逻辑操作。Tool Calling 打破了这个限制。当模型发现仅靠自己的内部知识无法回答问题时，它会暂停输出人类语言，转而输出一段机器可读的代码（通常是 JSON 格式），告诉外部程序：“去帮我查一下这个数据”。

#### 运行机制

Tool Calling 不是大模型自己在后台偷偷去联网或计算，而是**大模型与开发者代码（应用层）之间的一场“传球游戏”**。一个完整的周期通常包含 4 个步骤：

1. **第一步：环境注入（注入工具说明书）** 开发者在将用户的输入发送给大模型时，会在 System Prompt 中附带一份“工具列表”。这份列表详细描述了每个工具的名字、功能以及需要传入什么参数（基于 JSON Schema 格式）。
   - *示例输入：* “今天东京的天气怎么样？” + 工具库：`[{name: "get_weather", description: "获取指定城市的天气", parameters: {city: "string"}}]`
2. **第二步：模型决策与触发（大模型传球给应用）** 大模型阅读输入后，判断需要使用天气工具。它不会直接回答用户，而是输出一段结构化的 JSON 指令。
   - *模型输出：* `{"tool_calls": [{"name": "get_weather", "arguments": {"city": "Tokyo"}}]}`
3. **第三步：工具执行（应用查数据）** 开发者的代码拦截到这段 JSON，发现这是一个工具调用请求。代码在本地或服务器上执行真正的 API 请求，获取结果。
   - *工具返回结果：* `{"temperature": 22, "condition": "Sunny"}`
4. **第四步：结果整合与最终回复（应用传球回大模型）** 开发者将上一步的结果，作为“工具的观察结果（Observation）”再次发送给大模型。大模型读取这些实时数据后，整理成自然语言回复用户。
   - *模型最终输出：* “今天东京的天气是晴天，气温大概 22 度哦。”



#### 模型是怎么被训练出这种能力的？

早期的基座模型（如 GPT-3）并不懂什么是 JSON，也不懂什么时候该调工具。Tool Calling 能力主要是通过后训练（Post-training）阶段赋予的，核心包含以下几个步骤：

##### 1. 构造“交错格式”的微调数据 (SFT)

研究人员会人工构造或用更强的模型生成大量包含工具调用的对话轨迹（Trajectories）。数据中不仅包含人类语言，还强制嵌入了特定的控制字符和 JSON 格式。 训练数据会被整理成类似这种极度结构化的形式让模型学习：

> **[User]** 帮我算一下 1234 乘以 5678。 **[Thought]** 这是一个大数乘法，我容易算错，应该调用计算器工具。 **[Action]** `<tool_call> {"name": "calculator", "args": {"expr": "1234*5678"}} </tool_call>` **[Observation]** `7006652` **[Response]** 1234 乘以 5678 的结果是 7006652。

通过大量的 Supervised Fine-Tuning (SFT)，模型学会了这种“遇到问题 -> 思考 -> 输出格式化请求 -> 接收结果 -> 回答”的特定条件反射。

##### 2. 格式约束与惩罚 (Format Enforcement)

Tool Calling 最怕的是大模型“胡编乱造”参数格式，或者输出残缺的 JSON。 在训练过程中，损失函数（Loss Function）往往会对特定的格式 Token 给予更高的关注。有些框架在微调时会使用特殊的掩码（Masking），强制模型严格按照提供的 JSON Schema 生成参数，如果括号没闭合或者参数类型错了，就会在训练时受到惩罚。

##### 3. 强化学习对齐 (RLHF / RLAIF)

仅仅学会格式还不够，模型还需要学会“时机”。 通过强化学习（如 PPO 或 GRPO），给模型设定打分机制（Reward Model）：

- **加分项：** 面对需要实时信息的问题，果断调用搜索 API；传递的参数精准无误。
- **扣分项：** 幻觉调用（编造了一个不存在的工具）；过度调用（问“你好”也去查一遍搜索）；死循环调用（工具一直报错，模型一直死磕）。
...
![structure](./Note.assets/LLM-structure.jpg)



![structure-moe](./Note.assets/LLM-structure-moe.jpg)
